...
用于查询 Qiniu LAS 区域列表的数据源。底层调用 Manager 用户接口 `GET /v1/regions/list`（带 `resource` / `language` 查询参数），返回聚合了 `resource` 能力的区域列表。若仅需区域 id 与名称，见 HTTP API 文档 `GET /v1/regions` 与 `GET /v1/regions/{id}`（`docs/api/resource/manager/region.md`）。
