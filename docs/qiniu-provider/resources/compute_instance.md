---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "qiniu_compute_instance Resource - qiniu"
subcategory: ""
description: |-
  用于创建和管理Qiniu LAS实例的 resource
---

# qiniu_compute_instance (Resource)

用于创建和管理Qiniu LAS实例的 resource



<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `image_id` (String) 镜像ID
- `instance_type` (String) 实例类型，指定实例的规格
- `system_disk_size` (Number) 系统盘大小，单位是GiB

### Optional

- `cost_auto_renew_enabled` (Boolean) 自动续费开关，仅在 cost_charge_type 为 PrePaid 时生效
- `cost_auto_renew_period` (Number) 自动续费时长，仅在 cost_auto_renew_enabled 为 true 时生效
- `cost_auto_renew_period_unit` (String) 自动续费时长单位，仅在 cost_auto_renew_enabled 为 true 时生效，支持 Month、Year
- `cost_charge_type` (String) 实例计费类型
- `cost_discount_activity_id` (String) 预付费促销活动 ID，仅在 cost_charge_type 为 PrePaid 时生效
- `cost_period` (Number) 预付费购买时长，仅在 cost_charge_type 为 PrePaid 时生效；未显式指定时默认值为 1
- `cost_period_unit` (String) 预付费购买时长单位，仅在 cost_charge_type 为 PrePaid 时生效，支持 Month、Year；未显式指定时默认值为 Month
- `description` (String) 实例描述
- `disable_public_ip` (Boolean) 是否启用公网带宽但不分配独立公网 IP。未配置时由服务端推断：internet_public_ip_type 为 Shared 时为 true，否则为 false。
- `hostname` (String) 主机名称，指定实例的主机名
- `internet_charge_type` (String) 网络计费类型，取值范围：Bandwidth（按固定带宽计费）、PeakBandwidth（按峰值带宽计费）、Traffic（按流量计费）
- `internet_max_bandwidth` (Number) 公网最大带宽，单位是Mbps，取值范围0-300，默认值为0，表示不分配公网IP
- `internet_public_ip_type` (String) 公网 IP 类型，取值范围：Dedicated（独立公网 IP）、Shared（共享公网 IP）、Elastic（弹性公网 IP）。
- `name` (String) 实例名称，指定实例的名称
- `password` (String, Sensitive) 实例密码，如果未指定则随机生成。
- `placement_group_id` (String) 置放组ID，指定实例所属的置放组，如果不指定，则不加入任何置放组
- `security_group_ids` (Set of String) 安全组 ID 集合，实例一旦设置过安全组了，后续必须至少绑定一个安全组，不能是空安全组了
- `state` (String) 实例状态，取值为 Running 或 Shutdown
- `subnet_id` (String) 实例所属子网 ID。配置后实例将以 VPC 网络创建。
- `system_disk_type` (String) 系统盘类型，指定系统盘的存储类型
- `timeouts` (Block, Optional) (see [below for nested schema](#nestedblock--timeouts))
- `user_data` (String, Sensitive) 用户数据，指定实例的初始化脚本或配置

### Read-Only

- `cpu` (Number) 主机核心数量
- `created_at` (String) 实例创建时间，格式为RFC3339
- `expired_at` (String) 实例过期时间，格式为RFC3339，仅在预付费实例时返回
- `id` (String) 实例ID
- `image_name` (String) 镜像名称
- `memory` (Number) 主机内存大小，单位是GiB
- `network_type` (String) 实例网络类型，取值为 Standard、VPC 或 Isolated
- `order_id` (String) 实例创建订单 ID，仅在预付费实例时返回
- `order_state` (String) 实例创建订单状态，仅在预付费实例时返回
- `private_ip_addresses` (Attributes List) (see [below for nested schema](#nestedatt--private_ip_addresses))
- `public_ip_addresses` (Attributes List) (see [below for nested schema](#nestedatt--public_ip_addresses))
- `region_id` (String) 实例所在的区域ID
- `region_name` (String) 区域名称
- `system_disk_id` (String) 系统盘ID
- `vpc_id` (String) 实例所属 VPC ID，VPC 网络时回填

<a id="nestedblock--timeouts"></a>
### Nested Schema for `timeouts`

Optional:

- `create` (String) 一个可被解析为持续时间的字符串（参考 time.ParseDuration 文档），由数字和单位后缀组成，例如 "30s" 或 "2h45m"。有效的时间单位包括："s"（秒） "m"（分钟） "h"（小时）
- `delete` (String) 一个可被解析为持续时间的字符串（参考 time.ParseDuration 文档），由数字和单位后缀组成，例如 "30s" 或 "2h45m"。有效的时间单位包括："s"（秒） "m"（分钟） "h"（小时）。在销毁操作发生之前，将超时更改保存到状态中时，删除操作才适用。
- `read` (String) 一个可被解析为持续时间的字符串（参考 time.ParseDuration 文档），由数字和单位后缀组成，例如 "30s" 或 "2h45m"。有效的时间单位包括："s"（秒） "m"（分钟） "h"（小时）。读取操作发生在任何刷新或计划操作期间，当启用刷新时。
- `update` (String) 一个可被解析为持续时间的字符串（参考 time.ParseDuration 文档），由数字和单位后缀组成，例如 "30s" 或 "2h45m"。有效的时间单位包括："s"（秒） "m"（分钟） "h"（小时）


<a id="nestedatt--private_ip_addresses"></a>
### Nested Schema for `private_ip_addresses`

Read-Only:

- `ipv4` (String) 内网IPv4地址


<a id="nestedatt--public_ip_addresses"></a>
### Nested Schema for `public_ip_addresses`

Read-Only:

- `bandwidth` (Number) 公网IP带宽，单位是Mbps
- `id` (String) 公网IP地址ID
- `ipv4` (String) 公网IPv4地址
- `traffic` (Number) 公网IP流量，单位是GB
