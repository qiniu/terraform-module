...
test -f "$module_dir/execution/cluster/templates/mysql_packages.sh"
test -f "$module_dir/execution/cluster/templates/mysql_data_storage.sh"
test -f "$module_dir/execution/cluster/templates/mysql_data_storage_destroy.sh"
rg -q 'chmod 0644 /etc/mysql/mysql\.conf\.d/zz-innodb-cluster\.cnf' "$module_dir/execution/cluster/templates/mysql_innodb_node.sh"
rg -q 'datadir = /data/mysql' "$module_dir/execution/cluster/templates/mysql_innodb_node.sh"
rg -q 'rm -f /data/mysql/auto\.cnf' "$module_dir/execution/cluster/templates/mysql_innodb_node.sh"
rg -q 'MySQL did not load the InnoDB Cluster configuration' "$module_dir/execution/cluster/templates/mysql_innodb_node.sh"
rg -q 'data "qiniu_compute_region" "current"' "$module_dir/infrastructure/main.tf"
rg -q 'data "qiniu_compute_images" "available_official_images"' "$module_dir/infrastructure/main.tf"
test ! -f "$module_dir/image_data.tf"
rg -q 'system_disk_type   = "cloud.ssd"' "$module_dir/infrastructure/main.tf"
rg -q '当前区域不支持 EBS 云盘' "$module_dir/infrastructure/main.tf"
rg -q 'resource "qiniu_compute_disk" "mysql_data"' "$module_dir/infrastructure/main.tf"
rg -q 'resource "qiniu_compute_disk_attachment" "mysql_data"' "$module_dir/infrastructure/main.tf"
rg -q 'mysql_data_disk_size and mysql_data_disk_ids are mutually exclusive' "$module_dir/infrastructure/main.tf"
