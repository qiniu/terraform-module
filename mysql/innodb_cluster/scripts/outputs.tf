...
output "cluster_reconcile_command" {
  value = templatefile("${path.module}/templates/mysql_cluster_reconcile.sh", {
...
output "router_bootstrap_commands" {
  value = {
    for node_key, node in var.mysql_nodes :
