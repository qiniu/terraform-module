...
log_file=/var/log/las-dsh-installer.log
...
required_packages=(ca-certificates curl moreutils)
missing_packages=()
...
for command in curl base64 sha256sum tee ts; do
...
run_install 2>&1 | ts '[%Y-%m-%d %H:%M:%S]' | tee "${log_file}"
