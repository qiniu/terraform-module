...
require_regex "$skill_step" '^[[:space:]]*run:[[:space:]]*bash[[:space:]]+apps/deepseek-harness/modules/installer/tests/deployment-environment-skill\.sh[[:space:]]*$' 'workflow runs deployment-environment skill test'
...
require_text "$readme_text" '/home/dsh/.agents/skills/deployment-environment/SKILL.md' 'README documents the user deployment-environment skill path'
