...
require_assignment "$installer_block" 'source' '"\./modules/installer"' 'root remains on the legacy installer until the migration gate passes'
...
require_text "$readme_text" '迁移门禁未满足' 'README documents the blocked Ansible bootstrap migration'
require_text "$readme_text" '/bin/bash: Argument list too long' 'README records the instance_exec capacity failure'
require_text "$readme_text" 'modules/ansible-installer' 'README documents the retained Ansible bootstrap module'
