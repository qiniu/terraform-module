...
- 只修改明确配置字段、常量或注释且不改变执行逻辑的变更，不强制采用 TDD；仍需执行格式检查、模块验证和适用的契约测试。
...
- 用户要求创建 PR 时，默认向 `upstream`（`qiniu/terraform-module`）发起，使用 `origin` fork 中的同名分支作为来源；PR 标题必须遵循 `type(scope): summary` 格式（例如 `fix(deepseek-harness): 修复公网访问`），正文使用中文。若目标仓库或来源分支不同，先向用户确认。
